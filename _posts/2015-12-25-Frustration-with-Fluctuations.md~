...
If our time for averaging is long enough that \\(\Delta^2 \bar{f} \to 0 \\) (provided that those integrals we talked about converge of course), then we can equate the expectation value of the fluctuation(physical) with the variance(probabilistic)! Note that \\( \Delta^2 \bar{f} \leq \Delta^2 f\\) because \\(\langle \delta^2 f \rangle\\) can never be negative.
